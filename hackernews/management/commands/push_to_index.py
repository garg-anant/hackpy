...
from hackernews.models import NewsLinks
...
class Command(BaseCommand):
	...
	def recreate_index(self):
		...
		index_name = NewsLinks._meta.es_index_name
		if indices_client.exists(index_name):
			indices_client.delete(index=index_name)
		indices_client.create(index=index_name)
		indices_client.put_mapping(
			doc_type=NewsLinks._meta.es_type_name,
			body=NewsLinks._meta.es_mapping,
			index=index_name
		)

	def push_db_to_index(self):
		data = [
			self.convert_for_bulk(s, 'create') for s in NewsLinks.objects.all()
		]
